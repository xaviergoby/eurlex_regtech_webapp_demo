...
language_opt = st.sidebar.date_input("Please enter earliest date of publications (YYYY-MM-DD)").strftime('%Y-%m-%d')
...
res_limit = input_row1_col2.slider("Results to return (1-25)", 1, 25)
...
if input_row2_col2 == "ASC" and "DESC(?date)" in updated_query:
	updated_query = updated_query.replace("DESC(?date)", "ASC(?date)")
elif input_row2_col2 == "DESC" and "ASC(?date)" in updated_query:
