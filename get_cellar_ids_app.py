...
if __name__ == '__main__':
    ...
    # sparql_query = text_to_str('sparql_queries/custom_fintech_domain_sparql_13_06_2023.rq')
    # sparql_query = text_to_str(r'C:\Users\XGOBY\PycharmProjects\GeneralAIMLDLandEDA\eu_corpus_compiler\queries\sparql_queries\custom_fintech_domain_sparql_13_06_2023.rq')
    # sparql_query = text_to_str(r'C:\Users\XGOBY\PycharmProjects\GeneralAIMLDLandEDA\eu_corpus_compiler\queries\sparql_queries\custom_fintech_domain_sparql_13_06_2023_downsampled.rq')
    import streamlit as st


    st.title("EurLex Document Retrieval Via Sparql Queries")
    # descriptor_term_input = "financial_domain_sparql_2019-01-07_V2"
    # descriptor_term = "financial_domain_sparql_2019-01-07_V2"
    # descriptor_term = st.text_input(descriptor_term_input)
    descriptor_term = st.text_input("Sparql queries file name")
    # SPARQL Query from .rq file
    # sparql_query = text_to_str(fr'C:\Users\XGOBY\PycharmProjects\GeneralAIMLDLandEDA\eu_corpus_compiler\queries\sparql_queries\{descriptor_term}.rq')
    # sparql_query = text_to_str(fr'C:\Users\XGOBY\PycharmProjects\GeneralAIMLDLandEDA\eu_corpus_compiler\queries\sparql_queries\{descriptor_term}.rq')
    sparql_query = text_to_str(fr'C:\Users\XGOBY\PycharmProjects\eurlex_regtech_webapp_demo\queries\sparql_queries\{descriptor_term}.rq')

    # Get CELLAR information records from EU Sparql endpoint (json format)
    sparql_query_results = get_cellar_info_from_endpoint(sparql_query)
    print('RETURNED:', sparql_query_results)

    # for i, document in enumerate(sparql_query_results["results"]["bindings"], start=1):
    for i, res in enumerate(sparql_query_results["results"]["bindings"], start=1):
