...
class SparqlQueryBuilder:
	...
	def __init__(self):
		self.__prefixes = 'PREFIX cdm:<http://publications.europa.eu/ontology/cdm#>' \
		...
		self.__base_sparql_query = None
		
	
	def __build_query_selection(self, include_language=True, include_publication_date=True, include_celex=True, include_format=True):
		selection = 'SELECT' \
					' DISTINCT' \
		...
		if include_language is True:
			selection += '(GROUP_CONCAT(distinct ?langIdentifier;separator="|") as ?lang)'
		if include_publication_date is True:
			selection += '(GROUP_CONCAT(distinct ?date;separator="|") as ?workDate)'
		if include_format is True:
			selection += '(GROUP_CONCAT(distinct ?mtype;separator="|") as ?mtypes)'
		if include_celex is True:
			selection += '(GROUP_CONCAT(distinct ?celex;separator="|") as ?celexId)'
		return selection
			
			
	# def __build_query_where(self, init_publication_date="2020-01-01", include_language=True, include_publication_date=True, include_celex=True, include_format=True):
	def __build_query_where(self, init_publication_date="2020-01-01"):
		"""
		"2020-09-24"
		:param init_publication_date: yyy-mm-dd
		:param include_language:
		:param include_publication_date:
		:param include_celex:
		:param include_format:
		:return:
		"""
		# query_selection = self.__build_query_selection(include_language, include_publication_date, include_celex, include_format)
		# where = 'WHERE ' \
		# 		'{ graph ?g { ' \
		# 		'?work rdf:type ?resType . ' \
		# 		'?work cdm:resource_legal_id_celex ?celex . ' \
		# 		'?work cdm:work_date_document ?date . ' \
		# 		f'FILTER( ?date > {init_publication_date}^^xsd:date) ' \
		# 		'FILTER(  ?resType = cdm:act_preparatory || ?resType = cdm:decision_delegated || ?resType = cdm:decision_implementing || ' \
		# 		'?resType = cdm:decision || ?resType = cdm:directive || ?resType = cdm:guideline_ecb || ?resType = cdm:regulation_delegated || ' \
		# 		'?resType = cdm:regulation_implementing || ?resType = cdm:regulation ) ' \
		...
		graph_ga = 'graph ?ga { ' \
				   ...
				   '|| ?subject = <http://eurovoc.europa.eu/4813>  #	peoples bank ' \
		...
		graph_ge = 'graph ?ge { ' \
				   ...
				   'FILTER(lang(?title)="en" or lang(?title)="eng" or lang(?title)='' ). ' \
		...
		graph_gm = 'graph ?gm { ' \
				   ...
				   '}'
		where = 'WHERE { ' + graph_g + graph_ga + graph_ge + graph_gm + '}'
		return where
		
	def build_query(self, init_publication_date="2020-01-01", limit=10, include_language=True, include_publication_date=True, include_celex=True, include_format=True):
		query_selection = self.__build_query_selection(include_language, include_publication_date, include_celex, include_format)
		query_where = self.__build_query_where(init_publication_date)
		query = self.__prefixes + query_selection + query_where + f'GROUP BY ?work LIMIT {str(limit)}'
		return query
