"""
* Python module to create src cellar queries, query src for metadata of documents with sparql queries, and subsequently download associated documents and notices.
"""
import os
import re
...
class Eurlex:
  ...
  ?type=<http://publications.europa.eu/resource/authority/resource-type/PROP_DEC_NO_ADDRESSEE>)"""
    ...
    { ?eurovoc skos:prefLabel ?subjectLabel filter (lang(?subjectLabel)="en") }.}"""  # TODO - option to not filter by EN  here
        ...
        # add filter to only include latest version (inspired by src R package)
    ...
    def query_eurlex(
        ...
        """
        Query src for documents with a SPARQL query
        Parameters:
        -----------
        query: str
            ...
            if response.status_code == 200:
                ...
                if extract_caselaw_metadata:
                    ...
                    if re.match(
                        ...
                        if __name__ == "__main__":
                            print("No caselaw metadata extracted")
